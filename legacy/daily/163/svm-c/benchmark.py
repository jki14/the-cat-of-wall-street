...
def run(tday, logfile):
    ...
    with open('../data/dbf/2017.dbf', 'r') as dbf:
        ...
        logstr += 'tday = %s\n' % (history['record'].values()[0][tday][0])
        ...
        for code, table in history['record'].iteritems():
            ...
            if len([i for i in xrange(tday+1, tday+91) if None in table[i]])>0:
                continue
            # form mat & res
    ...
    with open('../data/dbf/2017.dbf', 'r') as dbf:
        ...
        for code, table in history['record'].iteritems():
            ...
            if len(table)<tday+181:
                continue
            # skip the stock has suspended in 90 trading days
            if len([i for i in xrange(tday, tday+91) if None in table[i]])>0:
                continue
            # mat++
            row = []
            ...
            hg = max(hg, 0.0)
            res = numpy.append(res, int(hg/(20.0/nlabels)))
            rat = numpy.append(rat, table[tday][seven])
            # tag++
            tag.append(code)
    if enscale:
        autoscale(mat, boundary)
