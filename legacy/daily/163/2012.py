...
try:
    with open('./data/dbf/2012.dbf', 'r') as dbf:
        foo = json.load(dbf)
except IOError:
...
for code in tracking_list:
    ...
    url = 'http://quotes.money.163.com/service/chddata.html?code=%s&start=%s&end=20171231&fields=TCLOSE;HIGH;LOW;TOPEN;LCLOSE;CHG;PCHG;TURNOVER;VOTURNOVER;VATURNOVER;TCAP;MCAP' % (code163, first)
    rep = None
    raw = None
    sup = 0.0
    while True:
...
with open('./data/dbf/2012.dbf', 'w') as dbf:
    json.dump(foo, dbf)
